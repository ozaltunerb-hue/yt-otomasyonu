...
from core.prompt_generator import (_generate_scenario, simplify_with_gate, NoValidScenarioError, validate_silent_visibility,
    validate_high_action, validate_beat3_ongoing_danger, validate_cast_size, validate_scenario_consistency, validate_ship_setting, score_scenario)
...
async def main(out_path):
    ...
    for i in range(5):
        ...
        g_ok, g_f = validate_ship_setting(sc, cat["forced_ship"])
        hist.append(key); used.append(key)
        ok = v_ok and a_ok and b_ok and c_ok and s_ok and g_ok
        rows.append({"n": i+1, "domain": cat["domain_id"], "camera": cam, "ship": cat["forced_ship"],
                     "event": cat["forced_event"], "accepted": ok, "failures": v_f + a_f + b_f + c_f + s_f + g_f,
                     "score": score_scenario(sc) if ok else None, "scenario": sc, "catalyst": cat})
        print(f"--- #{i+1} {cat['domain_id']} | {cam} | ok={ok} score={rows[-1]['score']} fail={v_f+a_f+b_f+c_f+s_f+g_f}", flush=True)
